...
int main() {
    ...
    CROW_ROUTE(app, "/api/relationships").methods(crow::HTTPMethod::GET)([]() {
        try {
            ...
            std::unique_ptr<sql::ResultSet> res(stmt->executeQuery("SELECT parent_id, child_id FROM relationships"));
            ...
            while (res->next()) {
                ...
                relationships_list.emplace_back(std::move(obj)); // Add each object to the list
            }

            result["relationships"] = std::move(relationships_list); // Move the list into the result

            return crow::response(200, result);
        } catch (sql::SQLException &e) {
            return crow::response(500, std::string("Error fetching relationships: ") + e.what());
        }
    });
